...
class SOW_Core {
	...
	**/
	 ...
	 *
	**/
	public function sanitizr($TYPE='POST', $array) {
		...
		switch($TYPE) {
			...
			default: 			die('sanitizr: $TYPE wrong or missing!');
		...
		foreach($array as $val=>$sanitizer_flag) {
			...
			// --
				...
				$return[$val] = (int) filter_input($S_TYPE, $val, FILTER_SANITIZE_NUMBER_INT);
			}
	...
	**/
	function cookie_set($cookie_name,$cookie_value,$expire=31536000,$serialize=false) { // 3600*24*365 = 1 year
		...
		if($serialize == true)
			setcookie($cookie_name , serialize($cookie_value) , time() + $expire, '/');
